...
class SparseGaussian3DRefinementModule(BaseModule):
    def __init__(
        ...
        self.preserve_semantic_prior = bool(preserve_semantic_prior)
    ...
    def forward(
        ...
        # Keep the semantic priors written into completed anchors alive through
        # refinement.  Previously only xyz was residual-refined, so the refine MLP
        # could overwrite box-completion class logits before GaussianHead saw them.
        if self.preserve_semantic_prior and self.semantic_dim > 0:
            sem_s = self.semantic_start
            sem_e = sem_s + self.semantic_dim
            if anchor.shape[-1] >= sem_e and output.shape[-1] >= sem_e:
                output = output.clone()
                output[..., sem_s:sem_e] = (
                    output[..., sem_s:sem_e]
                    + anchor[..., sem_s:sem_e].to(dtype=output.dtype) * self.semantic_prior_scale
                )
