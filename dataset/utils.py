...
def custom_collate_fn_temporal(instances):
    """Custom collate for temporal / Occ3D batches.

    This function supports per-GPU batch_size > 1.

    Why this is needed:
        Dense fields such as img, occ_label, occ_xyz, projection_mat have the
        same shape for every sample and can be stacked into a batch tensor.

        Point-cloud fields such as points and visibility_points have different
        number of points for each frame, e.g. [268987, 6] and [248210, 6].
        They must be kept as a Python list of per-sample tensors.  The training
        loop, LiDAR voxelizer, OctreeGaussianLifter and GaussianProbe path all
        consume per-sample point tensors correctly.

    Optional debug:
        Launch once with COLLATE_DEBUG=1 to print output field shapes.
    """
    import os
    ...
    # Variable-length fields.  Never torch.stack these.
    ...
    debug = os.environ.get('COLLATE_DEBUG', '0').lower() in {'1', 'true', 'yes', 'on'}

    def _same_tensor_shape(values):
        if len(values) == 0:
            return True
        if not all(isinstance(v, torch.Tensor) for v in values):
            return False
        shape0 = tuple(values[0].shape)
        return all(tuple(v.shape) == shape0 for v in values)

    def _same_numpy_shape(values):
        if len(values) == 0:
            return True
    ...
    def _collate(values, key=None):
        values = list(values)
        if len(values) == 0:
            return values

        # Keep variable-length point-like fields as list, so batch_size>1 works.
        ...
        if isinstance(v0, np.ndarray):
            if _same_numpy_shape(values):
                return torch.stack([torch.from_numpy(v) for v in values], dim=0)
            return values

        if isinstance(v0, torch.Tensor):
            if _same_tensor_shape(values):
        ...
        if isinstance(v0, dict):
            ...
            for sub_key in all_sub_keys:
                sub_values = [item[sub_key] for item in values if sub_key in item]
                out[sub_key] = _collate(sub_values, key=sub_key)
            return out

        if isinstance(v0, list):
            # Metadata lists are usually sample-specific; keep one list per sample.
            # For example, image file paths, sweeps, camera names, etc.
            return values
        ...
        # Numbers / bools can be converted to tensors when possible.
    ...
    for k in all_keys:
        vals = [instance[k] for instance in instances if k in instance]
        return_dict[k] = _collate(vals, key=k)

    if debug:
        debug_msg = {}
        for k, v in return_dict.items():
            if isinstance(v, torch.Tensor):
                debug_msg[k] = tuple(v.shape)
            elif isinstance(v, list):
                preview = []
                for item in v[:4]:
                    if isinstance(item, torch.Tensor):
                        preview.append(tuple(item.shape))
                    elif isinstance(item, np.ndarray):
                        preview.append(tuple(item.shape))
                    else:
                        preview.append(type(item).__name__)
                debug_msg[k] = {'list_len': len(v), 'preview': preview}
            elif isinstance(v, dict):
                debug_msg[k] = {sk: (tuple(sv.shape) if isinstance(sv, torch.Tensor) else type(sv).__name__) for sk, sv in v.items()}
            else:
                debug_msg[k] = type(v).__name__
        print('[custom_collate_fn_temporal DEBUG]', debug_msg, flush=True)
