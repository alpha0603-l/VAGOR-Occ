...
class OccupancyLoss(BaseLoss):
    ...
    def loss_voxel(self, pred_occ, sampled_xyz, sampled_label, occ_mask=None):
        ...
        # Auxiliary decoder supervision is training-only. During evaluation the
        # head intentionally returns an empty list to avoid a second renderer pass.
        # Return a device-local scalar tensor rather than Python 0.0 so MultiLoss
        # can detach/log it without a type error or host-side fallback.
        if pred_occ is None or len(pred_occ) == 0:
            if isinstance(sampled_xyz, torch.Tensor):
                return sampled_xyz.float().sum() * 0.0
            if isinstance(sampled_label, torch.Tensor):
                return sampled_label.float().sum() * 0.0
            return self.class_weights.float().sum() * 0.0

        tot_loss = 0.

        # Keep immutable references for multi-layer deep supervision.
        # With apply_loss_type="random_2" or "all", pred_occ contains more than one
        # decoder output.  The old code overwrote sampled_label after applying occ_mask
        # in the first loop, then tried to reuse the original [B, N] mask on the already
        # compressed [1, M] label tensor in the second loop, causing a shape mismatch.
        base_sampled_label = sampled_label
        base_sampled_xyz = sampled_xyz
        flat_occ_mask = None
        if occ_mask is not None:
            ...
            flat_occ_mask = occ_mask.flatten(1).bool()

        for semantics in pred_occ:
            cur_sampled_label = base_sampled_label
            cur_sampled_xyz = base_sampled_xyz

            if flat_occ_mask is not None:
                semantics = semantics.transpose(1, 2)[flat_occ_mask][None].transpose(1, 2)  # 1, C, M
                cur_sampled_label = base_sampled_label[flat_occ_mask][None]
                if cur_sampled_xyz is not None and cur_sampled_xyz.dim() >= 3:
                    cur_sampled_xyz = base_sampled_xyz[flat_occ_mask][None]

            loss_dict = {}
            # semantics = semantics.transpose(0, 1).unsqueeze(0)
            if self.use_focal_loss:
                loss_dict['loss_voxel_ce'] = self.loss_voxel_ce_weight * \
                    self.focal_loss(semantics, cur_sampled_label, cur_sampled_xyz, self.class_weights.type_as(semantics), ignore_index=255)
            else:
                loss_dict['loss_voxel_ce'] = self.loss_voxel_ce_weight * \
                    CE_ssc_loss(semantics, cur_sampled_label, self.class_weights.type_as(semantics), ignore_index=255)
            if self.use_sem_geo_scal_loss:
                loss_dict['loss_voxel_sem_scal'] = self.loss_voxel_sem_scal_weight * sem_scal_loss(semantics, cur_sampled_label, ignore_index=255)
                loss_dict['loss_voxel_geo_scal'] = self.loss_voxel_geo_scal_weight * geo_scal_loss(semantics, cur_sampled_label, ignore_index=255, non_empty_idx=self.empty_label)
            # Enter here
            if self.use_lovasz_loss:
                loss_dict['loss_voxel_lovasz'] = self.loss_voxel_lovasz_weight * lovasz_softmax(
                    torch.softmax(semantics, dim=1).transpose(1, 2).flatten(0, 1), cur_sampled_label.flatten(), ignore=self.lovasz_ignore)
            if self.use_dice_loss:
                loss_dict['loss_voxel_dice'] = self.dice_loss(semantics, cur_sampled_label)
